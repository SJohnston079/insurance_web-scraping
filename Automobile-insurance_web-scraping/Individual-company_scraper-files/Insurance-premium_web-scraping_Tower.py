...
def export_auto_dataset(input_indexes):
    ...
    tower_output_df.loc[:,"PolicyStartDate"] = test_auto_data_df.loc[:,"PolicyStartDate"]
...
# defining a function which take the information from the spreadsheet and formats it so it can be used to scrape premium from tower website
    ...
    def tower_auto_scrape_premium(data):
        ...
        try:
            ...
            try:
                ...
                Wait.until(EC.presence_of_element_located((By.ID, "underwritingDialog")))

                # printing out and returning an error message
                print("We're sorry The details you've given mean that we're currently unable to offer you cover.", end=" -- ")
                return "Doesn't Cover"
            except exceptions.TimeoutException:
                pass
    ...
    try:
        ...
        try: # checks if the reason our code failed is because the 'we need more information' pop up appeareds
            ...
            print("Need more information", end= " -- ")
            tower_output_df.loc[person_i, "Tower_Error_code"] = "Webiste Does Not Quote For This Car Variant/ Person"
        except exceptions.TimeoutException:
