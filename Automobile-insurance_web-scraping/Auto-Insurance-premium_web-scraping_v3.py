...
def auto_scape_all():
    ...
    ## getting a number for how many cars we wish to scrape from
    print("\nHow many cars do you wish scrape premiums for?")
    num_cars = ""

    # repeatedly prompt the user until is given a valid number of cars to scrape from
    while not isinstance(num_cars, int):
        num_cars = input(f"Enter EITHER (A number between 1 and {len(test_auto_data)} inclusive) OR ('ALL' to scrape for all examples in the test_auto_data1): ").upper()

        # attempting to convert the number of cars to an integer
        try: 
            if num_cars == "ALL": # check if num_cars is a valid string (set it to the number of examples in the dataset)
                num_cars = len(test_auto_data)
            else:
                num_cars = int(num_cars) # attempt to convert num_cars into an integer value
                
                # if num_cars is able to be converted into an integer, is the integer invalid (larger than the number of examples in the spreadsheet)
                if num_cars > len(test_auto_data):
                    raise ValueError
        except: # if the num_cars variable is invalid in some way, then set it to an empty string (to prompt again for input)
            num_cars = ""
