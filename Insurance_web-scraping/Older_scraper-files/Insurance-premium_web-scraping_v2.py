...
def auto_scape_all():
    ...
    def run_company_scrapers(person_i, company):
        ...
        if company == "AA":
            scraped_results = aa_auto_scrape(person_i)
        elif company == "AMI":
            scraped_results = ami_auto_scrape(person_i)
        elif company == "Tower":
            scraped_results = tower_auto_scrape(person_i)

        # ensuring
        if scraped_results != None:
            adjusted_agreed_value, monthly_premium, yearly_premium = scraped_results[0], scraped_results[1], scraped_results[2]
        else:
            return
...
def main():
    ...
    for key in test_auto_data:
        test_auto_data['PolicyStartDate'] = date.today()
