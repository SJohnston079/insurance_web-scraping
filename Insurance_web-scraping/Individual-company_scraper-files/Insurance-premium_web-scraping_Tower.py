...
def export_auto_dataset(num_datalines_to_export = len(test_auto_data_xlsx)):
    auto_dataset_for_export = tower_output_df.head(num_datalines_to_export) # get the given number of lines from the start
    auto_dataset_for_export.to_csv(f"{parent_dir}\\Individual-company_data-files\\tower_scraped_auto_premiums.csv", index=False)
...
# defining a function which take the information from the spreadsheet and formats it so it can be used to scrape premium from tower website
    def tower_auto_data_format(person_i):
        ...
        tower_data = {"Registration_number":test_auto_data.loc[person_i,'Registration'],
                    ...
                    "Policy_start_date":str(test_auto_data.loc[person_i, "PolicyStartDate"])
                    }
    ...
    try:
        ...
        if tower_auto_premium != None: # if an actual result is returned
